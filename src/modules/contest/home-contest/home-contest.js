...
export class HomeContest {
    ...
    getMyContests() {
        this.contestService
            .getMyContests(
                this.limitMyContests,
                this.pageMyContests,
                this.authService.getUserId(),
                this.sortMyContests,
                this.byMyContestsQuery
            )
            .then((data) => {
                ...
                if (this.totalPagesMyContests > 0) {
                    for (let i = 0; i < data.data.length; i++) {
                        this.myContests.push(
                            new Contest(
                                data.data[i].title,
                                data.data[i].description,
                                data.data[i].init_date,
    ...
    cambiarInstitucion(institucion) {
        document.getElementById("elegirInst").value = " ";
        if (institucion == " ") {
            this.idInstitution = " ";
            this.institucion = " ";
            this.allContests = [];
    ...
    filterByInstitution(idInstitution) {
        ...
        this.institutionService
            .getContestsByInstitution(idInstitution)
            .then((data) => {
                this.totalPagesAllContests = data.meta.totalPages;
                this.allContests = [];
                if (this.totalPagesAllContests > 0) {
